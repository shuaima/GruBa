...
For simplicity, assuming the microblogs that a user can \ret{} come from those owned by her/his followees.
...
\begin{definition}
\label{def:blog}
A microblog $M_b = (O, T, M, flag)$ has its owner $O$ (a.k.a. user in this paper) to whom $M_b$ belongs (either \twd{} or \retd{}), the generated time $T$ of $M_b$, the message content $M$, and $flag$ denoting $M_b$ is \retd{} or originally \twd{} by $O$. Here we use 1 and 0 to denote \retd{} and  \tw{}, respectively.
\end{definition}
