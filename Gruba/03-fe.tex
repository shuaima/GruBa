...
\begin{itemize}
	...
	\item values showing the average number of \retd{} and \twe{} microblogs per week: $\#W_r$, $\#W_t$
	\item a normalized vector regarding the time distribution of a user's \retg{} behavior: $P_{rt}\ = (p'_0,\ p'_1,\ ...,\ p'_{11})$, where $p'_0$ is the probability that the \retg{} activity happens from 0am to 2am, $p'_1$ is the probability that the \retg{} activity happens from 2am to 4am, and so on.
    \item a normalized vector regarding the time distribution of a user's \twtg{} behavior: $P_{tt}$, obtained as $P_{rt}$.

	\item a normalized vector with respect to the gap distribution of a user's \retg{} behavior: $P_{rg}\ = (p''_0,\ p''_1,\ ...,\ p''_{5})$, in which $p''_0$ is the probability that the gap between two \retd{} microblogs is within 1 min. Ditto for $p''_1$ (1 min to 1 hour), $p''_2$ (1 to 12 hours), $p''_3$ (12 to 24 hours), $p''_4$ (24 to 48 hours) and  $p''_5$ (more than 48 hours).
    \item a normalized vector with respect to the gap distribution of a user's \twtg{} behavior: $P_{tg}$, obtained as $P_{rg}$.
\end{itemize}

Hence, \textit{Behavior Feature} $H$ per user comes with:
\begin{equation}
\label{eq:beha}
	H = (\#B_u, R_{oc},\ \#W_r,\ P_{rt},\ P_{rg},\ \#W_t,\ P_{tt},\ P_{tg})
\end{equation}
...
In short, with a given lexicon(made by some professionals) consisting of several \textit{topics}, the interest feature of a user is a normalized vector, in which each dimension refers to the degree that the said user is interested in the corresponding \textit{topic}.
...
\begin{equation}
\label{eq:fu-flat}
	F(u) = (G_u,\ P_u, \#R_s, \#E_s, R_{ee}, U_t, \#B_u,\ R_{oc},\ \#W_r,\ P_{rt},\ P_{rg},\ \#W_t,\ P_{tt},\ P_{tg}, P_f)
