...
If $sim(b, t)$ satisfies a certain threshold, topic $t$ is labeled to blog $b$; 
...
W_t = \{(w'_i, p'_i)\}
\end{equation}
where $w'_i$ refers to a word in \textit{tag} $W_t$, and $p'_i$ is the probability that $w'_i$ appears in blogs with the said \textit{tag}, with $\sum_{\substack{i}} p'_i = 1$.
%
...
Among the $n$ dimensions of $S(u,\ L)$, those with top $x$ similarity values are selected to label the implicit interests of user $u$, which results an $x$ dimensional vector $P_f$ as described in Definition \ref{def:inte}.
