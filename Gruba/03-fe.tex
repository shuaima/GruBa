...
\stitle{Step 1:} Each blog of $u$ is decomposed into a word set, i.e., $b = \{w\}$ where $b \in B_s(u)$.
...
\stitle{Step 3:} A metric \textit{TF-IDF weight} $W_f$ is computed, i.e., employing TF-IDF (term-frequency and inverse document-frequency) to calculate the weight distribution of words in blog $b$:
\begin{equation}
\label{eq:tf}
W_f = \{(w_i, p_i)\}
\end{equation}
...
\refe{}
to result the word weight distribution of blog $b$.
...
W_t = \{(w'_i, p'_i)\}
\end{equation}
where $w'_i$ refers to a word in \textit{tag} $W_t$, and $p'_i$ is the probability that $w'_i$ appears in blogs with the said \textit{tag}, with $\sum_{\substack{i}} p'_i = 1$ ($|W_t|\ =\ 30$ in this work by default).
%
...
\end{equation}
where $W_f = \{(w_i, p_i)\}$, $t = \{c_j\}$, and $sim(w_i, t)$ is the averaged word similarity $sim(w_i, c_j)$ returned by word2vector \refe{}.
Go to \textbf{Step 7}.
...
\end{equation}
where $W_w = \{(w'_i, p'_i)\}$, $t = \{c_j\}$, and $sim(w'_i, t)$ is the averaged word similarity $sim(w'_i, c_j)$ returned by word2vector \refe{}.
Go to \textbf{Step 7}.
...
\stitle{Step 7:} Hence, the similarity of a blog $b$ against a lexicon topic $t$ is given by:
\begin{equation}
\label{eq:simbt}
...
\stitle{Step 8:} Repeat the above steps (Step 1 to Step 7) for the blog $b$ over every topic in lexicon, i.e., $\forall t_k \in L$ results one similarity value of $sim(b, t_k)$.
