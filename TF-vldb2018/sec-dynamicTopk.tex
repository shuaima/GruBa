...
\stitle{(1) Localization with pattern fragmentation}. We say that \{$P_{f1}(V_{f1},V_{f1})$, $\ldots$, $P_{fh}(V_{fh},V_{fh})$, $C$\} is an {\em $h$-fragmentation} of pattern $P(V_{P}$, $E_{P})$, denoted as ${\cal P}_{h}$,
...
%\vspace{-0.5ex}
\stitle{(3) Algorithm \inc}. We now provide a unified incremental solution to handle both pattern and data updates.
