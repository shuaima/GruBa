...
\stitle{Experimental Settings.} We use the following settings.

%\subsection{Experimental Settings}
%We used the following real-life and synthetic datasets.
\etitle{Data graphs.} We used a real-life and a synthetic dataset.
...
%firstly proposed the team formation problem;
...
%which has the same goal with our algorithms; and
...
All the findings on \youtube are reported in the Appendix.

%\subsection{Experimental Results}

\stitle{Experimental Results.} In all the experiments, we set $k=10$, $r=2$, $h=3$, $(|V_{P}|,$ $|E_{P}|)$ to be (10,12),
...
\stitle{Exp-1: Efficiency of \optgrouprec}. We firstly evaluated the performance of \optgrouprec vs. \mindia, \minsumdis and \denalk.

Algorithms \mindia, \minsumdis and \denalk do not scale well on large graphs.
Indeed,  (1) \mindia and \minsumdis take more than $8$ hours to finish their preprocessing, \ie computing all-pair-shortest-paths, and (2) \denalk takes more than 24 hours even when $k=1$ on \citationd.
By contrast, \optgrouprec takes around 100 seconds on \citationd by default settings.
...
\stitle{Exp-2: Effectiveness of \optgrouprec}.  We generated pattern graphs for \optgrouprec, and the corresponding queries (labels requirements) for \mindia, \minsumdis and \denalk, and manually checked the quality of matches returned by them. 
\looseness=-1
...
\warn{These verify that \optgrouprec can effectively preserve structural and capacity constraints for top-$k$ team formation \wrt edge and capacity satisfiability, and pertains a good team collaboration compatibility \wrt density and diameter.}
...
\etitle{(i) Pattern updates}. We generated $5$ sets of hybrid pattern updates, varying the number of updates of each set from $1$ to $7$.
...
\etitle{(ii) Data updates}. The setting is same as above. Varying the the amount of hybrid data updates in each set from 4\% to 24\%,
...
\etitle{(iii) Simultaneous pattern and data updates}. Using the same setting and varying the simultaneous pattern and data updates $(\Delta P, \Delta G)$ from (4.5\%, 4\%) to (27\%, 24\%) in Fig.~\ref{fig-exp-hyb-datapat-multi},
