...
An HR manager may help to set up a team for developing a new software product,
and she searches $G_1$ (without dashed edges) for proper candidates.
The desired team has
(1) one \kw{PM}, and one to two \kw{BAs}, \kw{UDs}, \kw{SAs}, \kw{SDs} and \kw{STs}, such that
(2) the \kw{PM} should collaborate with \kw{SAs}, \kw{BAs} and \kw{UDs} well, and the \kw{SDs} and \kw{STs} should collaborate with each other well and both with \kw{SAs} well.
...
\revise{One can verify that none of existing methods suffice to identify the above desired team.
They can only find teams satisfying the skill requirement~\cite{Lappas09,Kargar11,GajewarS12} and the lower bound capacity requirement~\cite{GajewarS12,realTeamForm13} in search condition (1),
while cannot guarantee  the specific collaboration relationships among team members, \ie structural constraints in search condition (2) (see details in Example~\ref{exa-teamF}).}
 \end{example}
...
\vspace{1.0ex}
Finding desired teams typically needs a lot of labor efforts, and it is even common for professionals to refine search conditions multiple times in order to find ideal teams~\cite{SajjadPG12,HabibiP15}. 
Moreover, real-life graphs are often big and constantly evolve over time~\cite{FanWW13-tods}. That is, team formation is accompanied with a highly dynamic environment.
%
\revise{However, it is often too costly to recompute top-$k$ teams from scratch in response to pattern and data updates.
...
It is to improve response time by reducing computations on big $G$ to small $\Delta G$ and $\Delta O$.}%%%EAT

\begin{example}
\label{exm-motivation-inc}
Recall $P_{1}$ and $G_{1}$ in Example~\ref{exm-motivation} and continue.
	
\sstab{(1)} When the manager get the match result, \ie the top-$k$ teams $P_{1}(G_{1})$ to $P_{1}$ in $G_{1}$, 
she may find $P_1$ is too restrictive to find possible matches.
Therefore, she enters a pattern update $\Delta P_1$ on $P_1$, composed of an edge 
...
\sstab{(2)} When a data update $\Delta G_1$ comes on $G_1$, composed of an edge insertion $(\kw{SD_3},\kw{ST_3})^+$.
...
\warn{To our knowledge, no previous work has studied pattern updates for incremental pattern matching~\cite{FanHT17,FanWW13-tods}},
...
\revise{
\stab (1) We propose {\em team simulation}, a revision of traditional graph pattern matching, for top-$k$ team formation problem. 
It extends existing methods by incorporating the structural and capacity constraints using pattern graphs (Section~\ref{sec-tsim}).
We formulate the dynamic top-$k$ team formation problem, to handle separate and simultaneous pattern and data updates,
in order to cope with the highly dynamic environment of team formation (Section~\ref{sec-tsim}).
\warn{To the best of our knowledge, this work gives the first investigation on pattern and simultaneous pattern and data incremental computations.} 
}

\revise{
\stab (2) We develop a \warn{quadruplicate} batch algorithm with two optimization techniques for computing top-$k$ teams via team simulation.} 
