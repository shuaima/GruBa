...
\newcommand{\rgraphsim}{\kw{undigSim}}
...
\revise{
We first propose {\em team simulation}, an extension of traditional graph pattern matching.
...
\vspace{0.5ex}
\ni(2) for each node $u\in V_P$, the number of nodes $v\in V_s$  with $(u, v)\in \Reps$ falls into $f_P(u)$.
...
Consider $P_1$ and $G_1$ in Fig.~\ref{exm-motivation}.
When team simulation with $r=2$ is adopted, $P_1$ matches $G_1$ as there is a perfect subgraph in $G_1$,
\ie the connected component of $G_1$ containing $\kw{PM_{1}}$, which resides in ball
$\ball{[\kw{PM_1}, 2]}$.
It maps \kw{PM}, \kw{BA}, \kw{UD}, \kw{SA}, \kw{SD} and \kw{ST} in $P_1$ to \kw{PM_1}, \kw{BA_1}, \{\kw{UD_1}, \kw{UD_2}\}, \{\kw{SA_1}, \kw{SA_2}\}, \{\kw{SD_1}, \kw{SD_2}\} and \{\kw{ST_1}, \kw{ST_2}\}, respectively.

One can easily verify that
(a) $P_1\eps \ball{[\kw{PM_{1}}, 2]}$, and the ball is exactly the match graph here,
and (b) the capacity bounds on all pattern nodes are satisfied.
\end{example}
\looseness=-1
...
\revise{
\stitle{Remarks}.
(1) Team simulation differs from graph simulation and strong simulation in the existence of capacity bounds on pattern graphs and its ability to capture matches on undirected graphs.
%
(2) As for locality constraint, different from strong simulation that requires the matched nodes to locate in a ball with a fixed given radius, team simulation adopts a more natural setting that the radius of the balls is flexible, only less than a user specified upper bound.
...
\label{subsec-teamF}
 ...
 Given $P$, $G$ and two positive integers $r$, $k$, the {\em top-$k$ team formation} problem, denoted as \teamF{(P, G, k)},
is to find a list $L_{k}$ of $k$ perfect subgraphs (\ie teams) with the top-$k$ largest density in $G$ for $P$, via team simulation.
\looseness=-1

Here the {\em density}\, $\density{G}$ of graph $G(V, E)$ is $|E|/|V|$, where $|E|$ and $|V|$ are the number of edges and the number of nodes respectively, \revise{as commonly used in data mining applications~\cite{maximumDenseSubgraph,EVMK12}}.
...
We simply set $k=1$, as most existing algorithms for \teamF{} only compute the best team, instead of top-$k$ teams.

One may want to look for candidate teams with existing methods, satisfying the search conditions in Example \ref{exm-motivation}:
(a) by minimizing the {\em team diameter} \cite{Lappas09},
which returns the team with $\{\kw{BA_3}$, \kw{PM_3}, \kw{UD_4}, \kw{SA_4}, \kw{SD_4}, $\kw{ST_4}\}$;
(b) by minimizing the {\em sum of all-pair distances} of teams \cite{Kargar11},
which returns exactly the same team as (a) in this case;
or (c) by maximizing the {\em team density} \cite{GajewarS12}, which returns the team consisting of the nodes of the two connected components in $G_1$ with \kw{PM_1} and \kw{BA_3} (excluding \kw{UD_2}, \kw{PM_3}, \kw{UD_4}, \kw{SA_4}).

However, one may notice that these teams only satisfy the skill requirement, \ie the search condition (1) in Example \ref{exm-motivation}, and cannot guarantee  the specific collaboration relationships among team members, \ie structural constraints. 
Indeed, the team found in (a) and (b) is connected by \kw{BA_3} only, and the team found in (c) has loose collaborations among its members.
...
satisfying both search condition (1) and (2), better than the teams found by the existing methods as shown above.
\end{example}}
...
\stitle{Pattern updates}. There are five types of pattern updates:
%
...
\stitle{Data updates}. There are four types of data updates, which can be
...
\stitle{Dynamic top-$k$ team formation}. Given $P$, $G$, two positive integers $r$, $k$, the list $L_{k}(P,G)$ of top-$k$ perfect subgraphs for $P$ in $G$,
...
It is worth mentioned that the \dynteamF{} problem covers a broad range of dynamic conditions,
