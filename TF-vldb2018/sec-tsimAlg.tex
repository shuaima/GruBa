...
\revise{
We devise a \warn{quadruplicate} time algorithm \grouprec for \teamF, with one novel feature and two optimizations, \ie pattern satisfiability check, and incremental computing for radius varied balls and density based filtering optimization, compared with traditional algorithms for strong simulation~\cite{MaCFHW14}.}
...
However, it is extremely costly to construct all such balls, \ie $r|V|$ balls, to compute perfect subgraphs residing in them. 
...
otherwise, it computes the perfect subgraph $G_{s}$ of $P$ in $\ball{[v,r]}$ via team simulation by invoking \rgraphsim (line 7, see Appendix),
an adaption from graph simulation~\cite{infsimu95,FanLMTWW10} and checking capacity bounds (line 8).
It then computes perfect subgraphs $G'_{s}$ of $P$ in inner balls $\ball{[v,t]}$ by invoking \incsim, an extension of the incremental algorithm in \cite{FanWW13-tods} and checking capacity bounds (lines 9-11, see Appendix).}
