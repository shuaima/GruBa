...
\identifyaffball filters out the balls that cannot match all fragments,
and there are no data updates on the ball.
\eop}
...
\incd is overall in
$O(\bigcup_{\widehat{G\oplus\Delta G}\in \affballsx}\bigcup_{i\in[1,h]}$ $r|M(P_{fi}, \widehat{G\oplus\Delta G})|$+$r|P||\affballsx|)$ time \wrt $\Delta G$, while $r$ is small, \ie 2 or 3  (See Appendix).
\looseness=-1
...
One can verify that procedure \identifyaffball is in $O(|\affballsx|)$ time,
\incmatch is in $O(|P||\affballsx|)$ time, and \comb is in $O(\bigcup_{i\in[1,h]}|M(P_{fi}, \widehat{G\oplus\Delta G})|)$ time.
Thus, algorithm \incd is in
$O(\bigcup_{\widehat{G\oplus\Delta G}\in \affballsx} \bigcup_{i\in[1,h]}|M(P_{fi}, \widehat{G\oplus\Delta G})|+|P||\affballsx|)$ time \wrt $\Delta G$.}%%%EAT
