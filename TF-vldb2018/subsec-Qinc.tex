...
\incp is in $O(\bigcup_{\hat{G}\in \affballsx}\bigcup_{i\in[1,h]}(|M(P_{fi}, \hat{G})|$ + $r|M(P_{fi} \oplus \Delta P_{fi}, \hat{G})|$) + $r|P \oplus \Delta P||\affballsx|$+$|\Delta P|)$ time \wrt $\Delta P$, while $r$ is small, \ie 2 or 3 (See Appendix).


\eat{
\stitle{Correctness \& complexity analysis}. The correctness of \incp \wrt $\Delta P$ is assured by
the correctness of (a) \identifyaffball (by Proposition~\ref{prop-canaffballs});
(b) \incmatch, which is assured by the correctness of \rgraphsim that has been proved previously, and of \patedgeinsert. Indeed, \patedgeinsert only removes
...
\comb is in $O(|\bigcup^{h}_{i=1} M(P_{fi}\oplus\Delta P_{fi},\ball{[v,r]})|+|V_{P\oplus \Delta P}||E_{\ball{[v,r]}}|)$ time.

Putting these together, algorithm \incp is in
$O(|\Delta P|$ + $\bigcup_{\hat{G}\in \affballsx}\bigcup_{i\in[1,h]}(|M(P_{fi}, \hat{G})|+|M(P_{fi} \oplus \Delta P_{fi}, \hat{G})|) + |P \oplus \Delta P||\affballsx|)$ time \wrt $\Delta P$.
}%%%EAT
