...
\revise{This is because the condition holds as long as
(a) the $i$-th ($i\in [1, h]$) bit of $fc_{j\Delta}$ is 0, \ie there exists an edge/node deletion on ${P}_{fi}$; or
(b) the $i$-th ($i\in [1, h]$) bit of $fc_{j\Delta}$ and $tc_j$ are both 1, \ie  balls with $tc_j$ already match with ${P}_{fi}$, although there are no edge/node deletions on ${P}_{fi}$.}
...
\sstab(1) $\Delta P_1$ comes with a unit edge deletion $\delta_1$=$(\kw{SD},$ $\kw{ST})^-$ on $P_{f_2}$, \revise{and all four $fc$ in \bfc are updated from $fc$ (1, 1) to $fc_{\Delta}$ (1, 0), as shown in the second column of \bfc in Fig.~\ref{fig-auxiliary-structures}(b).}

\revise{Procedure \identifyaffball firstly identifies ball $\ball{[\kw{PM_1},2]}$ with $tc$ $(1, 1)$ and $\ball{[\kw{PM_2},2]}$ with $tc$ $(1, 0)$ as \affballsx, 
