...
\revise{This is because \affballsx are balls that have the possibility to match all updated pattern fragments.}
The condition $tc_j\& fc_{j\Delta} = fc_{j\Delta}$ holds as long as
(a) the $i$-th ($i\in [1, h]$) bit of $fc_{j\Delta}$ is 0, \ie there exists an edge/node deletion on ${P}_{fi}$, \revise{which may produce more matched nodes}, or
...
Procedure \identifyaffball firstly identifies ball $\ball{[\kw{PM_1},2]}$ with $tc$ $(1, 1)$ and $\ball{[\kw{PM_2},2]}$ with $tc$ $(1, 0)$ as \affballsx,
